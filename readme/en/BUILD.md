...
```
akkara/
├── common/              # Basic primitives (ByteBufferL, LE, types)
├── format-api/          # Format API (BlockPacker, StripeWriter, etc.)
├── format-akk/          # AKK implementation (AkkBlockPacker, parity coders)
├── engine/              # Storage engine (MemTable, WAL, SSTable, etc.)
└── akkaradb/            # Integration module (Fat JAR generation)
```
...
```
akkara/plugin/
├── akkara-plugin/       # Gradle plugin
└── akkara-compiler/     # Kotlin compiler plugin (IR transformation)
```
