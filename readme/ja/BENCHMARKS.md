...
| パス         | ベンチ      |  キー数 | ValueSize |     ops/sec | p50 (µs) | p90 (µs) | p99 (µs) | 備考            |
|:-----------|:---------|-----:|----------:|------------:|---------:|---------:|---------:|:--------------|
...
- **MemTableヒット**: CPU律速、極めて高速
- **SSTヒット（hot）**: Bloomフィルター効果大
- **SSTヒット（cold）**: ディスクI/O待ち
...
|      Bloomあり |      Bloomなし | False Positive率 |
|-------------:|-------------:|----------------:|
| 95,238 ops/s | 18,518 ops/s |           0.98% |
...
1. `walGroupN`を増やす（バッチ効果）
   ```kotlin
   walGroupN = 512-1024
   ```

2. `walGroupMicros`を調整（耐久性とのトレードオフ）
   ```kotlin
...
4. L0ファイル数を減らす（コンパクション頻度を上げる）
   ```kotlin
   maxPerLevel = 2  // デフォルト4
   ```

**結果:**

- SST cold read: 150 µs → 80 µs（推定）
...
1. MemTableフラッシュ閾値を下げる
   ```kotlin
   thresholdBytesPerShard = 32 * 1024 * 1024 / shardCount  // 32MB
   ```
