...
namespace akkaradb::binpack::detail {
    ...
    [[nodiscard]] inline std::string_view memberName() noexcept {
        #if defined(_MSC_VER)
        const std::string_view sig = __FUNCSIG__; const auto ampLt = sig.rfind("<&"); if (ampLt == std::string_view::npos) { return {}; }
        const auto gt = sig.find('>', ampLt); if (gt == std::string_view::npos) { return {}; } const auto cc = sig.rfind("::", gt); if (cc
            == std::string_view::npos || cc < ampLt) { return {}; } return sig.substr(cc + 2, gt - cc - 2);
