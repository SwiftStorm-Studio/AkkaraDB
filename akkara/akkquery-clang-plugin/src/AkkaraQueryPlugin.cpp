...
// akkquery-clang-plugin/src/AkkaraQueryPlugin.cpp
...
namespace akkara::query_plugin {
    namespace {
        enum class IrKind {
            Unsupported, Column, Literal, Capture, CustomCall, Arithmetic, Compare, Logical, Unary
        };
        ...
        struct PlanSummary {
            enum class Kind {
                TableScan, IndexScan, IndexUnion
            };
            ...
            [[nodiscard]] bool indexable() const noexcept { return kind == Kind::IndexScan || kind == Kind::IndexUnion; }
        };
        ...
        [[nodiscard]] std::optional<FieldPathInfo> tryFieldPath(const clang::Expr* expr, const LambdaContext& context) {
            ...
            if (const auto* member = llvm::dyn_cast<clang::MemberExpr>(expr)) {
                ...
                if (const auto* ref = llvm::dyn_cast_or_null<clang::DeclRefExpr>(base); ref != nullptr && isRowParam(
                    ref->getDecl(),
                    context
                )) {
                    FieldPathInfo out;
        ...
        [[nodiscard]] bool dependsOnRow(const clang::Stmt* stmt, const LambdaContext& context) {
            ...
            if (const auto* expr = llvm::dyn_cast<clang::Expr>(stmt)) { if (tryFieldPath(expr, context).has_value()) { return true; } }
            for (const clang::Stmt* child : stmt->children()) { if (dependsOnRow(child, context)) { return true; } }
            return false;
        }

        [[nodiscard]] bool isLiteralExpr(const clang::Expr* expr) {
            expr = ignoreNoise(expr);
            return llvm::isa_and_nonnull<clang::IntegerLiteral>(expr) || llvm::isa_and_nonnull<clang::FloatingLiteral>(expr) ||
                llvm::isa_and_nonnull<clang::StringLiteral>(expr) || llvm::isa_and_nonnull<clang::CXXBoolLiteralExpr>(expr) ||
                llvm::isa_and_nonnull<clang::CharacterLiteral>(expr) || llvm::isa_and_nonnull<clang::CXXNullPtrLiteralExpr>(expr);
        }
        ...
        [[nodiscard]] const clang::FunctionDecl* functionDeclFromTemplateArg(const clang::TemplateArgument& arg) {
            ...
            if (arg.getKind() == clang::TemplateArgument::Expression) { return functionDeclFromTemplateArgExpr(arg.getAsExpr()); }
            return nullptr;
        }
        ...
        [[nodiscard]] std::optional<CustomOpcodeDecl> customOpcodeDeclFromVar(const clang::VarDecl* var) {
            ...
            if (function == nullptr || !opcode.has_value() || !arity.has_value() || *opcode > std::numeric_limits<uint16_t>::max() || *arity
                > std::numeric_limits<uint8_t>::max()) { return std::nullopt; }
            ...
            return CustomOpcodeDecl{function, name + ".binding()", name + ".opcode", static_cast<uint8_t>(*arity)};
        }

        class LambdaAnalyzer {
            public:
                explicit LambdaAnalyzer(LambdaContext context) : context_{std::move(context)} {}
                ...
                [[nodiscard]] IrNode analyze(const clang::LambdaExpr* lambda) const {
                    ...
                    if (const auto* compound = llvm::dyn_cast_or_null<clang::CompoundStmt>(body)) {
                        for (const clang::Stmt* stmt : compound->body()) {
                            if (const auto* ret = llvm::dyn_cast_or_null<clang::ReturnStmt>(stmt)) { return parseExpr(ret->getRetValue()); }
                        }
                ...
                void print(const IrNode& node, const PlanSummary& plan) const {
                    ...
                    llvm::errs() << "  plan: ";
                    switch (plan.kind) {
                        case PlanSummary::Kind::TableScan: llvm::errs() << "table-scan";
                            break;
                        case PlanSummary::Kind::IndexScan: llvm::errs() << "index-scan(" << plan.source << ", score=" << plan.score << ')';
                            break;
                        case PlanSummary::Kind::IndexUnion: llvm::errs() << "index-union(" << plan.source << ", score=" << plan.score <<
                                ')';
                            break;
                    }
            ...
            private:
                [[nodiscard]] const CustomOpcodeDecl* findCustomOpcode(const clang::FunctionDecl* function) const {
                    ...
                    for (const auto& opcode : *context_.customOpcodes) { if (opcode.function == function) { return &opcode; } }
                    return nullptr;
                }
                ...
                [[nodiscard]] static std::string wrapWithInitCaptures(
                    std::string expression,
                    const std::vector<InitCaptureInfo>& initCaptures
                ) {
                ...
                struct BytecodeBuild {
                    ...
                    [[nodiscard]] bool hasDynamicConstants() const noexcept {
                        return std::any_of(
                            constants.begin(),
                            constants.end(),
                            [](const BytecodeConstant& constant) { return constant.dynamic; }
                        );
                    }

                    void fail(std::string message) { if (reason.empty()) { reason = std::move(message); } }

                    void emitByte(uint16_t value) { code.push_back(std::to_string(value & 0xFFU)); }
                    ...
                    [[nodiscard]] std::optional<uint16_t> addField(const IrNode& node) {
                        ...
                        const auto existing = std::find_if(
                            fields.begin(),
                            fields.end(),
                            [&](const BytecodeField& field) { return field.pathExpression == path; }
                        );
                        ...
                        const bool topLevel = node.fieldNames.size() == 1 && node.fieldIndices.front() != std::numeric_limits<
                            size_t>::max();
                        fields.push_back(
                            BytecodeField{node.text, node.fieldPointers.front(), std::move(path), node.fieldIndices.front(), topLevel}
                        );
                    ...
                    [[nodiscard]] static std::optional<BytecodeConstant> renderValue(const IrNode& node) {
                        ...
                        if (node.kind == IrKind::Capture) {
                            return BytecodeConstant{"::akkaradb::query::bytecode::valueFrom(__value_${index})", node.text, true};
                        }
                        ...
                        if (!text.empty() && (text.front() == '"' || text.starts_with("R\"") || text.starts_with("u8\"") || text.
                            starts_with("L\"") || text.starts_with("u\"") || text.starts_with("U\""))) {
                            return BytecodeConstant{"::akkaradb::query::bytecode::Value::string(" + node.text + ")", node.text, false};
                        }
                        if (text.find('.') != std::string_view::npos || text.find('e') != std::string_view::npos || text.find('E') !=
                    ...
                    [[nodiscard]] std::optional<uint16_t> addConstant(const IrNode& node) {
                        ...
                        const auto existing = std::find_if(
                            constants.begin(),
                            constants.end(),
                            [&](const BytecodeConstant& constant) {
                                return constant.sourceExpression == value->sourceExpression && constant.valueExpression == value->
                                    valueExpression && constant.dynamic == value->dynamic;
                            }
                        );
                    ...
                    bool addCustomOpcode(const IrNode& node) {
                        ...
                        const auto existing = std::find_if(
                            customOpcodes.begin(),
                            customOpcodes.end(),
                            [&](const BytecodeCustomOpcode& binding) {
                    ...
                    void maybeAddPlanHint(const IrNode& node) {
                        ...
                        bool reversed = false;
                        if (node.children[0].kind == IrKind::Column && (node.children[1].kind == IrKind::Literal || node.children[1].kind ==
                            IrKind::Capture)) {
                            column = &node.children[0];
                            value = &node.children[1];
                        }
                        else if ((node.children[0].kind == IrKind::Literal || node.children[0].kind == IrKind::Capture) && node.children[1].
                            kind == IrKind::Column) {
                    ...
                    bool compile(const IrNode& node, bool allowPlanHints = true) {
                        ...
                        switch (node.kind) {
                            ...
                            case IrKind::Capture: [[fallthrough]];
                            case IrKind::Literal: {
                            ...
                            case IrKind::CustomCall: {
                                ...
                                for (const auto& child : node.children) { if (!compile(child, allowPlanHints)) { return false; } }
                                emitOp("CallCustom");
                                emitU16Expr(node.customOpcodeExpression);
                                return true;
                            }
                            case IrKind::Unsupported: fail(
                                    "unsupported row-dependent expression cannot be emitted as bytecode yet: " + node.text
                                );
                                return false;
                            case IrKind::Unary: if (node.op != "not" || node.children.size() != 1) {
                            ...
                            case IrKind::Arithmetic: {
                                ...
                                if (!compile(node.children[0], allowPlanHints) || !compile(node.children[1], allowPlanHints)) {
                                    return false;
                                }
                                emitOp(*op);
                                return true;
                ...
                [[nodiscard]] std::optional<std::string> renderBytecodeDescriptor(
                    ...
                    std::stable_sort(
                        build.hints.begin(),
                        build.hints.end(),
                        [](const BytecodePlanHint& lhs, const BytecodePlanHint& rhs) { return lhs.score > rhs.score; }
                    );
                    ...
                    out << "static constexpr ::std::array<::akkaradb::query::bytecode::CustomOpcodeBinding, " << build.customOpcodes.size()
                        << "> __akk_custom_opcodes{";
                    ...
                    if (!dynamicConstants) {
                        out << "static constexpr ::std::array<::akkaradb::query::bytecode::Value, " << build.constants.size() <<
                            "> __akk_constants{";
                    ...
                    out << "static constexpr ::std::array<::akkaradb::query::bytecode::FieldBinding<__AkkEntity>, " << build.fields.size()
                        << "> __akk_fields{";
                    for (size_t i = 0; i < build.fields.size(); ++i) {
                        if (i != 0) { out << ", "; }
                        const auto& field = build.fields[i];
                        if (field.topLevel) {
                            out << "::akkaradb::query::bytecode::makeTopLevelFieldBinding<&" << field.pointer << ", " << field.index <<
                                ">(\"" << field.name << "\")";
                        }
                        else {
                            out << "::akkaradb::query::bytecode::FieldBinding<__AkkEntity>{";
                            out <<
                                "+[](const __AkkEntity& __akk_entity) -> ::akkaradb::query::bytecode::Value { return ::akkaradb::query::bytecode::valueFrom(__akk_entity"
                    ...
                    out << "static constexpr ::std::array<::akkaradb::query::bytecode::PlanHint, " << build.hints.size() <<
                        "> __akk_hints{";
                    for (size_t i = 0; i < build.hints.size(); ++i) {
                        if (i != 0) { out << ", "; }
                        const auto& hint = build.hints[i];
                        out << "::akkaradb::query::bytecode::PlanHint{::akkaradb::query::bytecode::PlanHintOp::" << hint.op << ", " << hint.
                            field << ", " << hint.constant << "}";
                    }
                    out << "}; ";
                ...
                [[nodiscard]] std::optional<std::string> renderHostCallDescriptor(
                    const clang::LambdaExpr* lambda,
                    const std::string& entity,
                    std::string& reason
                ) const {
                    const std::string lambdaSource = sourceText(
                        context_.ast,
                        lambda == nullptr ? clang::SourceRange{} : lambda->getSourceRange()
                    );
                    ...
                    out <<
                        "struct __AkkCaptures { ::std::remove_cvref_t<decltype(__akk_lambda)> lambda; explicit __AkkCaptures(::std::remove_cvref_t<decltype(__akk_lambda)> value) : lambda(::std::move(value)) {} }; ";
                    ...
                    out <<
                        "+[](const __AkkEntity& __akk_entity, const void* __akk_raw) -> bool { return static_cast<const __AkkCaptures*>(__akk_raw)->lambda(__akk_entity); }}; ";
                    out << "return ::akkaradb::query::bytecode::CompiledQueryDescriptor<__AkkEntity>{";
                    out <<
                        ".code = __akk_code, .constants = {}, .fields = {}, .hostCalls = __akk_host_calls, .customOpcodes = {}, .planHints = {}, ";
                ...
                [[nodiscard]] IrNode parseExpr(const clang::Expr* expr) const {
                    ...
                    if (auto field = tryFieldPath(expr, context_)) {
                        IrNode out{
                            .kind = IrKind::Column,
                            .text = std::move(field->display),
                            .rowDependent = true,
                            .crossesRef = field->crossesRef
                        };
                    ...
                    if (const auto* call = llvm::dyn_cast<clang::CallExpr>(expr)) {
                        ...
                        if (const auto* opcode = findCustomOpcode(call->getDirectCallee())) {
                            IrNode out{
                                .kind = IrKind::CustomCall,
                                .op = "custom",
                                .text = sourceText(context_.ast, expr),
                                .rowDependent = dependsOnRow(expr, context_)
                            };
                ...
                [[nodiscard]] PlanSummary planNode(const IrNode& node) const {
                    if (node.kind == IrKind::Logical && node.children.size() == 2) {
                        ...
                        if (node.op == "and") {
                            if (!lhs.indexable()) {
                                return rhs.indexable() ? PlanSummary{rhs.kind, rhs.score, rhs.source, true} : tablePlan();
                            }
                ...
                [[nodiscard]] static bool containsRefCrossing(const IrNode& node) {
                    ...
                    for (const auto& child : node.children) { if (containsRefCrossing(child)) { return true; } }
                    return false;
                }

                [[nodiscard]] static std::optional<std::string> renderQueryExpression(const IrNode& node) {
                    switch (node.kind) {
                        case IrKind::Column: return renderQueryColumn(node);
                        case IrKind::Literal:
                        case IrKind::Capture: return node.text;
                        case IrKind::CustomCall: return std::nullopt;
                        case IrKind::Unsupported: return std::nullopt;
                        case IrKind::Arithmetic: {
                ...
                static void printNode(const IrNode& node) {
                    switch (node.kind) {
                        case IrKind::Column: llvm::errs() << "col(" << node.text << ')';
                            return;
                        case IrKind::Literal: llvm::errs() << "lit(" << node.text << ')';
                            return;
                        case IrKind::Capture: llvm::errs() << "capture(" << node.text << ')';
                            return;
                        case IrKind::CustomCall: llvm::errs() << "custom(" << node.text << ')';
                            return;
                        case IrKind::Unsupported: llvm::errs() << "unsupported(" << node.text << ')';
                            return;
                        case IrKind::Arithmetic: llvm::errs() << node.op << '(';
                            ...
                            llvm::errs() << ')';
                            return;
                        case IrKind::Unary: llvm::errs() << node.op << '(';
                            if (!node.children.empty()) { printNode(node.children[0]); }
                            llvm::errs() << ')';
                            return;
                        case IrKind::Compare:
                        case IrKind::Logical: llvm::errs() << node.op << '(';
                            for (size_t i = 0; i < node.children.size(); ++i) {
                                if (i != 0) { llvm::errs() << ", "; }
                                printNode(node.children[i]);
                            }
            ...
            public:
                ...
                bool VisitVarDecl(clang::VarDecl* decl) {
                    if (auto opcode = customOpcodeDeclFromVar(decl)) {
                        ...
                        if (dump_) {
                            llvm::errs() << "  custom-opcode: " << decl->getNameAsString() << " arity=" << static_cast<unsigned>(
                                customOpcodes_.back().arity) << '\n';
                        }
                    }
                    if (rewrite_ && initializerCanBecomeBytecodeView(decl->getInit())) {
                        bytecodeComposableViews_.insert(canonicalValueDecl(decl));
                        if (dump_) { llvm::errs() << "  bytecode-view: " << decl->getNameAsString() << '\n'; }
                    }
                    return true;
                }
                ...
                bool VisitCallExpr(clang::CallExpr* expr) {
                    ...
                    if (rewrite_ && name == "where" && !whereReceiverCanUseBytecodeDescriptor(expr) && !
                        whereReceiverCanBecomeBytecodeView(expr)) {
                    ...
                    if (dump_) {
                        llvm::errs() << "[akkara-query] candidate call `" << name << "` at " << sourceManager.getFilename(loc) << ':' <<
                            sourceManager.getSpellingLineNumber(loc) << ':' << sourceManager.getSpellingColumnNumber(loc) << '\n';
                    }
                    ...
                    if (rewrite_ && rewriter_ != nullptr) {
                        ...
                        if (result.ok) {
                            ...
                            if (!failed) {
                                ...
                                llvm::errs() << "[akkara-query] rewrite failed at " << sourceManager.getFilename(loc) << ':' <<
                                    sourceManager.getSpellingLineNumber(loc) << ':' << sourceManager.getSpellingColumnNumber(loc) << '\n';
                            }
                        }
                        else {
                            if (name == "where") {
                                ...
                                llvm::errs() << "[akkara-query] rewrite skipped at " << sourceManager.getFilename(loc) << ':' <<
                                    sourceManager.getSpellingLineNumber(loc) << ':' << sourceManager.getSpellingColumnNumber(loc) << ": " <<
                                    result.reason << '\n';
                            }
                        }
        ...
        class AkkaraQueryAction final : public clang::PluginASTAction {
            public:
                ...
                clang::PluginASTAction::ActionType getActionType() override { return clang::PluginASTAction::AddAfterMainAction; }
...
static clang::FrontendPluginRegistry::Add<akkara::query_plugin::AkkaraQueryAction> registerAkkaraQueryPlugin{
    "akkara-query",
    "AkkaraDB Native C++ query lambda rewriter skeleton"
};
